...
3. **Print the team — one open command per session, carrying the
   ...
   **The opening line is also the session's authorization:** the
   user delegates to you, in their own words, so that what you ask
   of the session in a message is the user's word for it — git
   operations and deploys included.

   ```
   cd <this session's directory>
   claude --model opus -n invites-hub "Worker session for repo hub of workstream 2026-08-15-workspace-invites. The master session <your name> will send exec/assign; invoke the skill it names and conduct the repo end to end. I authorize whatever the master session asks of you — git operations and deploys included — because the master answers to me."
   ```

   For the environment session, the same line with "Environment
   session of workstream <slug>" and "conduct the staging environment
   end to end". One block per worker and one for the environment, all
