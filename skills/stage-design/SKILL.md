...
**One mode, every wave, every time:**

- **Round 1 is the full ten** — the first look is where
  unknown-unknowns surface, so nothing narrows it.
- **Every later round is the delta**: pass `lenses: [...]` with **only
  the lenses that did not pass in the previous round**, plus a `scope`
  note naming what changed. The workflow returns that list as `open` —
  pass it back verbatim.
- **A lens that returns `pass` is finished.** It never runs again, in
  any later round.
- **Convergence is the only exit**: repeat until no lens is left open.
  There is no closing round, no round cap, and no re-run "to be sure".
...
in a previous round's own addition — the full re-run was auditing its
own fix surface, at ten reviewers a round.
...
5. Run the next round with `lenses` = the round's `open` list. Repeat
   from 1 until `open` is empty — **that is the close**.
...
wave), the verdict table (from `reviews.md`), the count of
...
`CLAUDE.md`). On "approved with fixes": the fold-in rule (§3) — one
