...
`Makefile` and engineering doctrine.

## How you judge

Walk every proof step of every brief:

- **A command that exists.** `run` is a `make` target or a test spec
  the recon or the `Makefile` names, with its arguments, or that the
  entry itself creates (and then the brief says so). `expect` is what
  it prints: the cases by name, the bad paths among them. "Works",
  "passes", "is done" alone are not proofs. The last step is `make
  verify` → exit 0.
...
>
> **Example, blocker** — proof: "run `make test`". The `Makefile` has
> no `test` target (recon: `check`, `verify`, `test-unit`,
> `test-integration`). Fix: `make test-integration pkg=orders` with the
> cases named.
...
- **Read the recon and the `Makefile`** before judging a command: the
...
`Makefile` target, the case names, the artboard); per finding, `says`
